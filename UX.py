...
class PageLogin:
    def __init__(self):
        ...
        # Login button
        self.login_button = Button(self.app, text="LOGIN", command=self.login_account.check_login, bg="orange",
                                   fg="red")
        self.login_button.place(x=276, y=212)

        # Register button
        self.register_button = Button(self.app, text="REGISTER", command=self.register_account.register, bg="orange",
                                      fg="red")
...
class HomePage:
    def __init__(self):
        ...
        balance_value = 5000  # Số dư cụ thể để hiển thị
        self.balance_entry = Entry(self.app, width=18, justify="center",font=("Arial",24))
        self.balance_entry.insert(0, balance_value)  # Chèn giá trị số dư vào ô vuông
        self.balance_entry.place(x=300, y=63)
